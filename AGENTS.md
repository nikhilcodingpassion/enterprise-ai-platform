...
- You should assume that all Enterprise AI Platform services are running. To verify, you can check the `backend/log` directory to
  make sure we see logs coming out from the relevant service.
- To connect to the Postgres database, use: `docker exec -it enterprise-ai-platform-relational_db-1 psql -U postgres -c "<SQL>"`
- When making calls to the backend, always go through the frontend. E.g. make a call to `http://localhost:3000/api/persona` not `http://localhost:8080/api/persona`
- Put ALL db operations under the `backend/enterprise-ai-platform/db` / `backend/ee/enterprise-ai-platform/db` directories. Don't run queries
  outside of those directories.

## Project Overview

**Enterprise AI Platform** is an open-source Gen-AI and Enterprise Search platform that connects to company documents, apps, and people. It features a modular architecture

### Background Workers (Celery)

Enterprise AI Platform uses Celery for asynchronous task processing with multiple specialized workers:
...
**Reason:** We have a very carefully curated selection of icons that match our Enterprise AI Platform guidelines. We do NOT want to muddy those up with different aesthetic stylings.
...
If you need help with this step, reach out to `raunak@Enterprise AI Platform.app`.
...
**Reason:** We've put in a lot of effort to unify the components that are rendered in the Enterprise AI Platform app. Using raw components breaks the entire UI of the application, and leaves it in a muddier state than before.
...
```typescript
// ✅ Good - Use custom Enterprise AI Platform color classes
...
There are 4 main types of tests within Enterprise AI Platform:

### Unit Tests

These should not assume any Enterprise AI Platform/external services are available to be called.
...
These tests assume that all external dependencies of Enterprise AI Platform are available and callable (e.g. Postgres, Redis,
MinIO/S3, Vespa are running + OpenAI can be called + any request to the internet is fine + etc.).

However, the actual Enterprise AI Platform containers are not running and with these tests we call the function to test directly.
...
Standard integration tests. Every test in `backend/tests/integration` runs against a real Enterprise AI Platform deployment. We cannot
...
These tests are an even more complete version of the Integration Tests mentioned above. Has all services of Enterprise AI Platform
...
to logs via the `backend/log/<service_name>_debug.log` file. All Enterprise AI Platform services (api_server, web_server, celery_X)
...
**Always raise `Enterprise AI PlatformError` from `Enterprise AI Platform.error_handling.exceptions` instead of `HTTPException`.
Never hardcode status codes or use `starlette.status` / `fastapi.status` constants directly.**

A global FastAPI exception handler converts `Enterprise AI PlatformError` into a JSON response with the standard
`{"error_code": "...", "detail": "..."}` shape. This eliminates boilerplate and keeps error
handling consistent across the entire backend.

```python
from Enterprise AI Platform.error_handling.error_codes import Enterprise AI PlatformErrorCode
from Enterprise AI Platform.error_handling.exceptions import Enterprise AI PlatformError

# ✅ Good
raise Enterprise AI PlatformError(Enterprise AI PlatformErrorCode.NOT_FOUND, "Session not found")

# ✅ Good — no extra message needed
raise Enterprise AI PlatformError(Enterprise AI PlatformErrorCode.UNAUTHENTICATED)

# ✅ Good — upstream service with dynamic status code
raise Enterprise AI PlatformError(Enterprise AI PlatformErrorCode.BAD_GATEWAY, detail, status_code_override=upstream_status)
...
Available error codes are defined in `backend/Enterprise AI Platform/error_handling/error_codes.py`. If a new error
...
```python
raise Enterprise AI PlatformError(Enterprise AI PlatformErrorCode.BAD_GATEWAY, detail, status_code_override=e.response.status_code)
```
